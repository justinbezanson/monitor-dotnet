    ...
    <Card class="w-full max-w-md shadow-xl border-border/50 bg-card/80 backdrop-blur-sm">
      ...
      <form @submit.prevent="handleLogin">
        <CardContent class="space-y-4">
          <div v-if="authStore.error" class="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-500 text-sm flex items-center gap-2">
        ...
        <CardFooter class="flex flex-col gap-4">
          ...
          <div class="text-sm text-center text-muted-foreground">
            ...
            <router-link to="/register" class="text-primary hover:underline font-semibold">Sign up</router-link>
          </div>
        </CardFooter>
      </form>
    </Card>
